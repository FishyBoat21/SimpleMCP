        ...
        if ($userPasskeys === []) {
            ...
            foreach ($userPasskeys as $pk) {
                ...
                $passkeysHtml .= '<tr>
                    ...
                    <td>
                        <form method="post" action="/account/passkey/delete" style="margin:0;">
            ...
            if (btn) {
                btn.addEventListener("click", async function() {
                    ...
                    try {
                        ...
                        var verifyRes = await fetch("/account/passkey/register/verify", {
                            method: "POST",
                            headers: { "Content-Type": "application/x-www-form-urlencoded" },
                        ...
                        if (verifyRes.ok && resData.success) {
                            window.location.href = "/account?msg=" + encodeURIComponent("Passkey registered successfully!");
                        } else {
        ...
        (function() {
            ...
            async function loginWithPasskey(conditional) {
                ...
                try {
                    ...
                    var optRes = await fetch("/account/passkey/login/options", {
                        method: "POST",
                        headers: { "Content-Type": "application/x-www-form-urlencoded" },
                    ...
                    var verifyRes = await fetch("/account/passkey/login/verify", {
                        method: "POST",
                        headers: { "Content-Type": "application/x-www-form-urlencoded" },
                        body: new URLSearchParams({
                            id: cred.id,
                    ...
                    if (verifyRes.ok && verifyData.success) {
                        window.location.href = verifyData.redirect_url || "/account";
                    } else {
